...
def _member_prompt(profile: HealthProfile, *, goal: str | None, meal_type: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "餐单完整性（重要，必须遵守）：全家共餐的晚餐/午餐必须给出 5-6 道独立菜品，包括至少 2 道荤菜/蛋白质主菜（如鸡肉、鱼虾、瘦猪肉、牛肉、鸭肉、羊肉等，每道是独立菜品，不能把多个食材合并成一道）、至少 2 道蔬菜（每道是独立菜品）、1 份主食（米饭/面条/馒头/杂粮饭等）、可以额外加 1-2 道汤或凉菜。单人正餐至少 2 道荤菜/蛋白质主菜 + 2 道蔬菜 + 主食。早餐至少包含主食 + 蛋白质来源（蛋/奶/豆）+ 1 道配菜。绝对不要把多个菜合并成一道（例如\u201c鸡胸肉配时蔬\u201d算 1 道，不算 2 道），每道菜必须单独列出。不要只给素菜或只给汤粥。",
...
def _family_prompt(profile: FamilyHealthProfile, *, goal: str | None, meal_type: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "份量表达：默认使用一小碗、一碗、一盘、一杯、一掌心、一个等日常说法；不要展开每个食材的克数、毫升数或配方级用量。",
            "只有用户明确要求精确克数、营养计算、热量估算或详细食谱时，才给克数；主食如果必须给重量，写熟饭大概份量，不写生米重量。",
            "餐单完整性（重要，必须遵守）：全家共餐的晚餐/午餐必须给出 5-6 道独立菜品，包括至少 2 道荤菜/蛋白质主菜（如鸡肉、鱼虾、瘦猪肉、牛肉、蛋、豆腐等，每道是独立菜品，不能把多个食材合并成一道）、至少 2 道蔬菜（每道是独立菜品）、1 份主食（米饭/面条/馒头/杂粮饭等）、可以额外加 1-2 道汤或凉菜。单人正餐至少 2 道荤菜/蛋白质主菜 + 2 道蔬菜 + 主食。早餐至少包含主食 + 蛋白质来源（蛋/奶/豆）+ 1 道配菜。绝对不要把多个菜合并成一道（例如\u201c鸡胸肉配时蔬\u201d算 1 道，不算 2 道），每道菜必须单独列出。不要只给素菜或只给汤粥。",
            f"餐单范围：{_meal_type_text(meal_type)}",
            f"本次目标：{goal or '无'}",
            "",
            "全家健康画像：",
